...
function render(templateId, data, opts = {}) {
  ...
  // Custom music: inject bgm src. Matches the whole opening tag (not just an
  // existing src="...") since the templates no longer ship a default src —
  // a stricter regex here silently stopped matching when that was removed.
  if (d.showMusic && opts.musicUrl) {
    html = html.replace(/<audio id="bgm"[^>]*>/, `<audio id="bgm" loop preload="auto" src="${opts.musicUrl}">`);
  }
